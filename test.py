...
def scrape_job_detail(url):
    ...
    try:
        ...
        update_date = (
            soup.select("div.basic-info__icon--last_updated_at")[0]
            .select_one("p.basic-info__last_updated_at")
            .get_text()[6:]
            .strip()
        )
        if update_date:
        ...
        for name, keyword in sections.items():
            ...
            for h in header_tags:
                ...
                if (
                    len(h.get_text(strip=True)) > 20
                ):  # Skip long paragraphs that just happen to contain the word
                    continue
